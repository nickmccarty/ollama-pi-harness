...
SYNTH_INSTRUCTION = (
    "Using the findings above, output ONLY the markdown document starting with # — no preamble, no commentary, no file path references, no explanation. Each section must include a concrete implementation example such as a working code snippet, a CLI command with real values, or a configuration file with specific settings. For each tool or technique, explicitly state the tool version, integration method, and a measurable outcome or configuration step. Ensure every item provides actionable details that demonstrate how to apply the concept in practice, including specific commands, code blocks, or configuration values. Include at least one specific, measurable result or outcome for each implementation step to improve depth and specificity. Structure each section with 'What', 'Why', and 'How' to enhance clarity and actionable detail. For each implementation step, specify the exact command, code, or config value that can be copied and executed. Where relevant, compare or contrast alternatives to highlight trade-offs and decision factors. **End each section with a brief bullet list of 2-3 concrete, measurable outcomes or results that validate the implementation.** Additionally, require that every section includes a real-world example with specific tool versions, command-line invocations, or configuration values that can be directly executed or applied. **For each section, explicitly state the tool version, integration method, and at least one measurable outcome or configuration step.** **Every section must contain at least one executable command, code block, or configuration snippet with real-world values.** **Every section must include a comparison or trade-off between at least two alternatives where relevant, to improve depth and specificity.** **Every section must contain a specific, actionable implementation step (code, CLI, config) that can be directly copied and executed.** **Every section must contain a measurable outcome or configuration step that directly validates the implementation.** **Every section must include a real, executable example with specific tool versions, command-line invocations, or configuration values that can be directly applied in practice.** **Every section must contain a concrete, executable example with specific tool versions, command-line invocations, or configuration values that can be directly applied in practice, and must include a measurable outcome or configuration step that directly validates the implementation.** **Every section must contain a specific, measurable result or outcome that directly validates the implementation step.**"
)
