...
SYNTH_INSTRUCTION = (
    "Using the findings above, output ONLY the markdown document starting with # — no preamble, no commentary, no file path references, no explanation. Each section must include a concrete implementation example such as a working code snippet, a CLI command with real values, or a configuration file with specific settings. For each tool or technique, explicitly state the tool version, integration method, and a measurable outcome or configuration step. Ensure every item provides actionable details that demonstrate how to apply the concept in practice, including specific commands, code blocks, or configuration values. Include at least one specific, measurable result or outcome for each implementation step to improve depth and specificity. Structure each section with 'What', 'Why', and 'How' to enhance clarity and actionable detail. For each implementation step, specify the exact command, code, or config value that can be copied and executed. Where relevant, compare or contrast alternatives to highlight trade-offs and decision factors. **End each section with a brief bullet list of 2-3 concrete, measurable outcomes or results that validate the implementation.** Every section must contain a specific, actionable implementation step (code, CLI, config) that can be directly copied and executed. Every section must contain a measurable outcome or configuration step that directly validates the implementation. Every section must explicitly name the tool, library, or framework being used, including version numbers where applicable. Every section must contain a step-by-step guide with at least one real-world command or code example that can be copy-pasted and run directly. Every section must include a comparison or trade-off analysis where relevant to improve specificity and depth. **Every section must contain a real-world example with specific tool versions, command-line invocations, or configuration values that can be directly executed or applied in practice.** Ensure that each section includes a measurable outcome or validation step that directly demonstrates the effectiveness of the implementation, such as a successful command execution, a configuration file change, or a test result. **For each section, include at least one concrete, copy-pasteable example with version numbers, command-line invocations, or configuration values that can be directly executed or applied in practice.** **Require that each section includes a specific, measurable outcome or validation step that directly demonstrates the effectiveness of the implementation, such as a successful command execution, a configuration file change, or a test result.** **Add a new requirement: each section must include a specific, measurable validation step that directly demonstrates the effectiveness of the implementation, such as a successful command execution, a configuration file change, or a test result.**"
)
